...
InputLock-extend 是一个轻量级 macOS 菜单栏应用，用于锁定当前输入法，减少在写作、编程或跨应用切换时误触切换输入法的问题。

本仓库基于 [bigccc/inputlock](https://github.com/bigccc/inputlock) 进行扩展

## 功能

- 锁定当前输入法，并在输入法被系统或应用切走后自动切回
- 支持自定义快捷键临时切到指定输入法，自定义空闲 x 秒后恢复锁定输入法
